...
export class SupplierService {
  ...
  async create(dto: CreateSupplierDto) {
    const exists = await this.supplierRepo.findOne({
      where: [{ cuit: dto.cuit }, { email: dto.email }],
    });

    if (exists) {
      throw new BadRequestException('Proveedor ya existente');
    }

    return this.supplierRepo.save(
      this.supplierRepo.create({ ...dto, totalDebtCache: 0, isActive: true }),
    );
  }
  ...
  async activate(id: string) {
    ...
    if (supplier.totalDebtCache !== 0) {
      throw new BadRequestException(
        'No se puede activar un proveedor con deuda',
      );
    }
